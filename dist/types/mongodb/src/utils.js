...
import { URL } from 'url';
import { ObjectId, resolveBSONOptions } from './bson';
import { MAX_SUPPORTED_WIRE_VERSION } from './cmap/wire_protocol/constants';
import { LEGACY_HELLO_COMMAND } from './constants';
import { MongoCompatibilityError, MongoInvalidArgumentError, MongoNotConnectedError, MongoParseError, MongoRuntimeError } from './error';
 ...
 * @internal
 */
export function checkCollectionName(collectionName) {
 ...
 * @internal
 */
export function normalizeHintField(hint) {
 ...
 * @internal
 */
export function parseIndexOptions(indexSpec) {
    const fieldHash = {};
    const indexes = [];
    let keys;
...
/** @internal */
export function filterOptions(options, names) {
    const filterOptions = {};
 ...
 */
export function applyRetryableWrites(target, db) {
    var _a;
 ...
 */
export function applyWriteConcern(target, sources, options) {
 ...
 */
export function isPromiseLike(maybePromise) {
 ...
 */
export function decorateWithCollation(command, target, options) {
 ...
 */
export function decorateWithReadConcern(command, coll, options) {
 ...
 */
export function decorateWithExplain(command, explain) {
    if (command.explain) {
        return command;
    }
 ...
 * @internal
 */
export function getTopology(provider) {
 ...
 */
export function defaultMsgHandler(name, option) {
 ...
 */
export function deprecateOptions(config, fn) {
...
/** @internal */
export function* makeCounter(seed = 0) {
    let count = seed;
    while (true) {
 ...
 * @internal
 */
export function maxWireVersion(topologyOrServer) {
    if (topologyOrServer) {
 ...
 */
export function collationNotSupported(server, cmd) {
 ...
 */
export function eachAsync(arr, eachFn, callback) {
    arr = arr || [];
    let idx = 0;
    let awaiting = 0;
...
/** @internal */
export function eachAsyncSeries(arr, eachFn, callback) {
    arr = arr || [];
    let idx = 0;
...
/** @internal */
export function arrayStrictEqual(arr, arr2) {
...
/** @internal */
export function errorStrictEqual(lhs, rhs) {
    if (lhs === rhs) {
        return true;
    }
    if (!lhs || !rhs) {
...
/** @internal */
export function makeStateMachine(stateTable) {
    return function stateTransition(target, newState) {
        const legalStates = stateTable[target.s.state];
        if (legalStates && legalStates.indexOf(newState) < 0) {
            throw new MongoRuntimeError(`illegal state transition from [${target.s.state}] => [${newState}], allowed: [${legalStates}]`);
        }
...
export function makeClientMetadata(options) {
    options = options !== null && options !== void 0 ? options : {};
    const metadata = {
        driver: {
            name: 'nodejs',
...
/** @internal */
export function calculateDurationInMs(started) {
 ...
 */
export function makeInterruptibleAsyncInterval(fn, options) {
    let timerId;
    let lastCallTime;
 ...
 * @internal
 */
export function resolveOptions(parent, options) {
    var _a, _b, _c;
    const result = Object.assign({}, options, resolveBSONOptions(options, parent));
    // Users cannot pass a readConcern/writeConcern to operations in a transaction
    const session = options === null || options === void 0 ? void 0 : options.session;
    if (!(session === null || session === void 0 ? void 0 : session.inTransaction())) {
        const readConcern = (_a = ReadConcern.fromOptions(options)) !== null && _a !== void 0 ? _a : parent === null || parent === void 0 ? void 0 : parent.readConcern;
        if (readConcern) {
            result.readConcern = readConcern;
        }
        const writeConcern = (_b = WriteConcern.fromOptions(options)) !== null && _b !== void 0 ? _b : parent === null || parent === void 0 ? void 0 : parent.writeConcern;
        if (writeConcern) {
            result.writeConcern = writeConcern;
        }
    }
    const readPreference = (_c = ReadPreference.fromOptions(options)) !== null && _c !== void 0 ? _c : parent === null || parent === void 0 ? void 0 : parent.readPreference;
    if (readPreference) {
...
export function isRecord(value, requiredKeys = undefined) {
 ...
 * @internal
 */
export function emitWarningOnce(message) {
 ...
 */
export function enumToString(en) {
 ...
 */
export function shuffle(sequence, limit = 0) {
...
export function commandSupportsReadConcern(command, options) {
