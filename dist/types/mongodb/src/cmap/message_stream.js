...
import { BinMsg, Msg, Response } from './commands';
import { compress, Compressor, decompress, uncompressibleCommands } from './wire_protocol/compression';
...
function processIncomingData(stream, callback) {
    ...
    decompress(compressorID, compressedBuffer, (err, messageBody) => {
        ...
        if (messageBody.length !== messageHeader.length) {
            callback(new MongoDecompressionError('Message body and message header must be the same length'));
            return;
        }
        // If we are a monitoring connection message stream and
        // there is more in the buffer that can be read, skip processing since we
