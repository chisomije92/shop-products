...
import { MongoInvalidArgumentError, MongoMissingCredentialsError, MongoRuntimeError, MongoServerError } from '../../error';
...
function executeScram(cryptoMethod, authContext, callback) {
    ...
    if (!authContext.nonce) {
        return callback(new MongoInvalidArgumentError('AuthContext must contain a valid nonce property'));
    }
...
function continueScramConversation(cryptoMethod, response, authContext, callback) {
    ...
    if (iterations && iterations < 4096) {
        callback(
        // TODO(NODE-3483)
        new MongoRuntimeError(`Server returned an invalid iteration count ${iterations}`), false);
        return;
    }
    const salt = dict.s;
    const rnonce = dict.r;
...
function resolveError(err, result) {
    ...
    if (result) {
        if (result.$err || result.errmsg)
            return new MongoServerError(result);
    }
    return;
}
