...
import { pluckBSONSerializeOptions } from '../bson';
import { CLOSE, CLUSTER_TIME_RECEIVED, COMMAND_FAILED, COMMAND_STARTED, COMMAND_SUCCEEDED, MESSAGE, PINNED, UNPINNED } from '../constants';
import { MongoCompatibilityError, MongoMissingDependencyError, MongoNetworkError, MongoNetworkTimeoutError, MongoRuntimeError, MongoServerError, MongoWriteConcernError } from '../error';
import { TypedEventEmitter } from '../mongo_types';
import { applySession, updateSessionFromResponse } from '../sessions';
import { calculateDurationInMs, maxWireVersion, MongoDBNamespace, now, uuidV4 } from '../utils';
import { CommandFailedEvent, CommandStartedEvent, CommandSucceededEvent } from './command_monitoring_events';
import { GetMore, KillCursor, Msg, Query } from './commands';
...
export class Connection extends TypedEventEmitter {
    constructor(stream, options) {
        ...
        this[kQueue] = new Map();
        this[kMessageStream] = new MessageStream(Object.assign(Object.assign({}, options), { maxBsonMessageSize: (_b = this.hello) === null || _b === void 0 ? void 0 : _b.maxBsonMessageSize }));
        this[kStream] = stream;
    ...
    getMore(ns, cursorId, options, callback) {
        ...
        if (wireVersion < 4) {
            ...
            const queryOptions = applyCommonQueryOptions({}, Object.assign(options, Object.assign({}, pluckBSONSerializeOptions(options))));
    ...
    killCursors(ns, cursorIds, options, callback) {
        ...
        if (maxWireVersion(this) < 4) {
            try {
                write(this, new KillCursor(ns.toString(), cursorIds), Object.assign({ noResponse: true }, options), callback);
            }
            catch (err) {
                callback(err);
            }
        ...
        this.command(ns, { killCursors: ns.collection, cursors: cursorIds }, Object.assign({ [kFullResult]: true }, options), (err, response) => {
            if (err || !response)
                return callback(err);
            ...
            if (!Array.isArray(response.documents) || response.documents.length === 0) {
                return callback(
                // TODO(NODE-3483)
                new MongoRuntimeError(`invalid killCursors result returned for cursor id ${cursorIds[0]}`));
            }
...
/** @event */
Connection.COMMAND_STARTED = COMMAND_STARTED;
/** @event */
Connection.COMMAND_SUCCEEDED = COMMAND_SUCCEEDED;
/** @event */
Connection.COMMAND_FAILED = COMMAND_FAILED;
/** @event */
Connection.CLUSTER_TIME_RECEIVED = CLUSTER_TIME_RECEIVED;
/** @event */
...
export class CryptoConnection extends Connection {
    ...
    command(ns, cmd, options, callback) {
        ...
        if (serverWireVersion < 8) {
            callback(new MongoCompatibilityError('Auto-encryption requires a minimum MongoDB version of 4.2'));
            return;
        }
...
/** @internal */
export function hasSessionSupport(conn) {
        ...
        operationDescription.cb = (err, reply) => {
            if (err) {
                ...
                if (reply && (reply.ok === 0 || reply.$err)) {
                    conn.emit(Connection.COMMAND_FAILED, new CommandFailedEvent(conn, command, reply, operationDescription.started));
                }
                else {
                    conn.emit(Connection.COMMAND_SUCCEEDED, new CommandSucceededEvent(conn, command, reply, operationDescription.started));
                }
            }
            if (typeof callback === 'function') {
                callback(err, reply);
