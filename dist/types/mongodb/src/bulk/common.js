import { Long, ObjectId, resolveBSONOptions } from '../bson';
import { MongoBatchReExecutionError, MONGODB_ERROR_CODES, MongoInvalidArgumentError, MongoServerError, MongoWriteConcernError } from '../error';
import { DeleteOperation, makeDeleteStatement } from '../operations/delete';
import { executeOperation } from '../operations/execute_operation';
import { InsertOperation } from '../operations/insert';
import { AbstractOperation } from '../operations/operation';
import { makeUpdateStatement, UpdateOperation } from '../operations/update';
import { PromiseProvider } from '../promise_provider';
import { applyRetryableWrites, getTopology, hasAtomicOperators, resolveOptions } from '../utils';
...
export function mergeBatchResults(batch, bulkResult, err, result) {
...
function executeCommands(bulkOperation, options, callback) {
    ...
    const finalOptions = resolveOptions(bulkOperation, Object.assign(Object.assign({}, options), { ordered: bulkOperation.isOrdered }));
    ...
    try {
        if (isInsertBatch(batch)) {
            executeOperation(bulkOperation.s.collection.s.db.s.client, new InsertOperation(bulkOperation.s.namespace, batch.operations, finalOptions), resultHandler);
        }
        else if (isUpdateBatch(batch)) {
            executeOperation(bulkOperation.s.collection.s.db.s.client, new UpdateOperation(bulkOperation.s.namespace, batch.operations, finalOptions), resultHandler);
        }
        else if (isDeleteBatch(batch)) {
            executeOperation(bulkOperation.s.collection.s.db.s.client, new DeleteOperation(bulkOperation.s.namespace, batch.operations, finalOptions), resultHandler);
        }
    }
    catch (err) {
...
export class FindOperators {
    ...
    update(updateDocument) {
        ...
        return this.bulkOperation.addToOperationsList(BatchType.UPDATE, makeUpdateStatement(currentOp.selector, updateDocument, Object.assign(Object.assign({}, currentOp), { multi: true })));
    }
    ...
    updateOne(updateDocument) {
        ...
        const currentOp = buildCurrentOp(this.bulkOperation);
        return this.bulkOperation.addToOperationsList(BatchType.UPDATE, makeUpdateStatement(currentOp.selector, updateDocument, Object.assign(Object.assign({}, currentOp), { multi: false })));
    }
    ...
    replaceOne(replacement) {
        ...
        const currentOp = buildCurrentOp(this.bulkOperation);
        return this.bulkOperation.addToOperationsList(BatchType.UPDATE, makeUpdateStatement(currentOp.selector, replacement, Object.assign(Object.assign({}, currentOp), { multi: false })));
    }
    /** Add a delete one operation to the bulk operation */
    deleteOne() {
        const currentOp = buildCurrentOp(this.bulkOperation);
        return this.bulkOperation.addToOperationsList(BatchType.DELETE, makeDeleteStatement(currentOp.selector, Object.assign(Object.assign({}, currentOp), { limit: 1 })));
    }
    /** Add a delete many operation to the bulk operation */
    delete() {
        const currentOp = buildCurrentOp(this.bulkOperation);
        return this.bulkOperation.addToOperationsList(BatchType.DELETE, makeDeleteStatement(currentOp.selector, Object.assign(Object.assign({}, currentOp), { limit: 0 })));
    }
...
export class BulkOperationBase {
    ...
    raw(op) {
        ...
        if ('replaceOne' in op || 'updateOne' in op || 'updateMany' in op) {
            if ('replaceOne' in op) {
                ...
                const updateStatement = makeUpdateStatement(op.replaceOne.filter, op.replaceOne.replacement, Object.assign(Object.assign({}, op.replaceOne), { multi: false }));
            ...
            if ('updateOne' in op) {
                ...
                const updateStatement = makeUpdateStatement(op.updateOne.filter, op.updateOne.update, Object.assign(Object.assign({}, op.updateOne), { multi: false }));
                if (!hasAtomicOperators(updateStatement.u)) {
                    throw new MongoInvalidArgumentError('Update document requires atomic operators');
                }
                return this.addToOperationsList(BatchType.UPDATE, updateStatement);
            ...
            if ('updateMany' in op) {
                ...
                const updateStatement = makeUpdateStatement(op.updateMany.filter, op.updateMany.update, Object.assign(Object.assign({}, op.updateMany), { multi: true }));
                if (!hasAtomicOperators(updateStatement.u)) {
                    throw new MongoInvalidArgumentError('Update document requires atomic operators');
                }
                return this.addToOperationsList(BatchType.UPDATE, updateStatement);
        ...
        if ('deleteOne' in op) {
            ...
            return this.addToOperationsList(BatchType.DELETE, makeDeleteStatement(op.deleteOne.filter, Object.assign(Object.assign({}, op.deleteOne), { limit: 1 })));
        }
        if ('deleteMany' in op) {
            if ('q' in op.deleteMany) {
                throw new MongoInvalidArgumentError('Raw operations are not allowed');
            }
            return this.addToOperationsList(BatchType.DELETE, makeDeleteStatement(op.deleteMany.filter, Object.assign(Object.assign({}, op.deleteMany), { limit: 0 })));
        }
    ...
    execute(options, callback) {
        ...
        if (this.s.batches.length === 0) {
            const emptyBatchError = new MongoInvalidArgumentError('Invalid BulkOperation, Batch cannot be empty');
