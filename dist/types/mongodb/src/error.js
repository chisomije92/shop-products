 ...
 * https://github.com/mongodb/specifications/blob/b07c26dc40d04ac20349f989db531c9845fdd755/source/server-discovery-and-monitoring/server-discovery-and-monitoring.rst#not-writable-primary-and-node-is-recovering
 */
export const LEGACY_NOT_PRIMARY_OR_SECONDARY_ERROR_MESSAGE = new RegExp('not master or secondary', 'i');
/**
 * @internal
...
export function isNetworkErrorBeforeHandshake(err) {
...
export function needsRetryableWriteLabel(error, maxWireVersion) {
    var _a, _b, _c;
    ...
    if (error instanceof MongoError) {
        if ((maxWireVersion >= 9 || error.hasErrorLabel(MongoErrorLabel.RetryableWriteError)) &&
...
function isRecoveringError(err) {
    ...
    return (LEGACY_NOT_PRIMARY_OR_SECONDARY_ERROR_MESSAGE.test(err.message) ||
...
export function isNodeShuttingDownError(err) {
 ...
 */
export function isSDAMUnrecoverableError(error) {
...
export function isNetworkTimeoutError(err) {
    return !!(err instanceof MongoNetworkError && err.message.match(/timed out/));
}
export function isResumableError(error, wireVersion) {
