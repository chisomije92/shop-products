...
import { APM_EVENTS, CLOSED, CMAP_EVENTS, CONNECT, DESCRIPTION_RECEIVED, ENDED, HEARTBEAT_EVENTS, SERVER_HEARTBEAT_FAILED, SERVER_HEARTBEAT_STARTED, SERVER_HEARTBEAT_SUCCEEDED } from '../constants';
import { isNetworkErrorBeforeHandshake, isNodeShuttingDownError, isSDAMUnrecoverableError, MongoCompatibilityError, MongoError, MongoErrorLabel, MongoInvalidArgumentError, MongoNetworkError, MongoNetworkTimeoutError, MongoServerClosedError, MongoUnexpectedServerResponseError, needsRetryableWriteLabel } from '../error';
import { Logger } from '../logger';
import { TypedEventEmitter } from '../mongo_types';
import { isTransactionCommand } from '../transactions';
import { collationNotSupported, makeStateMachine, maxWireVersion, supportsRetryableWrites } from '../utils';
import { STATE_CLOSED, STATE_CLOSING, STATE_CONNECTED, STATE_CONNECTING, TopologyType } from './common';
...
/** @internal */
export class Server extends TypedEventEmitter {
    /**
     * Create a server
     */
    constructor(topology, description, options) {
        ...
        this.s.pool.on(Connection.CLUSTER_TIME_RECEIVED, (clusterTime) => {
        ...
        monitor.on(Server.SERVER_HEARTBEAT_SUCCEEDED, (event) => {
            this.emit(Server.DESCRIPTION_RECEIVED, new ServerDescription(this.description.hostAddress, event.reply, {
...
/** @event */
Server.SERVER_HEARTBEAT_STARTED = SERVER_HEARTBEAT_STARTED;
/** @event */
Server.SERVER_HEARTBEAT_SUCCEEDED = SERVER_HEARTBEAT_SUCCEEDED;
/** @event */
Server.SERVER_HEARTBEAT_FAILED = SERVER_HEARTBEAT_FAILED;
/** @event */
Server.CONNECT = CONNECT;
/** @event */
Server.DESCRIPTION_RECEIVED = DESCRIPTION_RECEIVED;
/** @event */
...
function markServerUnknown(server, error) {
    ...
    server.emit(Server.DESCRIPTION_RECEIVED, new ServerDescription(server.description.hostAddress, undefined, {
        error,
...
function makeOperationHandler(server, connection, cmd, options, callback) {
    ...
    return function handleOperationResult(error, result) {
        ...
        if (error instanceof MongoNetworkError) {
            ...
            if ((isRetryableWritesEnabled(server.s.topology) || isTransactionCommand(cmd)) &&
