...
import { MongoAPIError, MongoCompatibilityError, MONGODB_ERROR_CODES, MongoError, MongoErrorLabel, MongoExpiredSessionError, MongoInvalidArgumentError, MongoRuntimeError, MongoServerError, MongoTransactionError, MongoWriteConcernError } from './error';
...
import { _advanceClusterTime, TopologyType } from './sdam/common';
import { isTransactionCommand, Transaction, TxnState } from './transactions';
import { calculateDurationInMs, commandSupportsReadConcern, isPromiseLike, maxWireVersion, maybePromise, now, uuidV4 } from './utils';
 ...
 * @public
 */
export class ClientSession extends TypedEventEmitter {
    /**
    ...
    constructor(client, sessionPool, options, clientOptions) {
        ...
        if (options.snapshot === true) {
            ...
            if (options.causalConsistency === true) {
                throw new MongoInvalidArgumentError('Properties "causalConsistency" and "snapshot" are mutually exclusive');
            }
        }
        this.client = client;
    ...
    get loadBalanced() {
        var _b;
        return ((_b = this.client.topology) === null || _b === void 0 ? void 0 : _b.description.type) === TopologyType.LoadBalanced;
    }
    /** @internal */
    ...
    pin(conn) {
        ...
        conn.emit(PINNED, this.inTransaction() ? ConnectionPoolMetrics.TXN : ConnectionPoolMetrics.CURSOR);
    }
    /** @internal */
    unpin(options) {
    ...
    endSession(options, callback) {
        ...
        const finalOptions = Object.assign({ force: true }, options);
    ...
    advanceClusterTime(clusterTime) {
        ...
        if (!clusterTime.clusterTime || clusterTime.clusterTime._bsontype !== 'Timestamp') {
            throw new MongoInvalidArgumentError('input cluster time "clusterTime" property must be a valid BSON Timestamp');
        }
        if (!clusterTime.signature ||
            ...
            throw new MongoInvalidArgumentError('input cluster time must have a valid "signature" property with BSON Binary hash and BSON Long keyId');
        }
    ...
    startTransaction(options) {
        ...
        if (isSharded(this.client.topology) &&
            topologyMaxWireVersion != null &&
            topologyMaxWireVersion < minWireVersionForShardedTransactions) {
            throw new MongoCompatibilityError('Transactions are not supported on sharded clusters in MongoDB < 4.2.');
        }
        // increment txnNumber
...
export function maybeClearPinnedConnection(session, options) {
    ...
    if (conn && topology != null) {
        ...
        if ((options === null || options === void 0 ? void 0 : options.error) == null || (options === null || options === void 0 ? void 0 : options.force)) {
            ...
            conn.emit(UNPINNED, session.transaction.state !== TxnState.NO_TRANSACTION
...
function isMaxTimeMSExpiredError(err) {
    ...
    return (err.code === MONGODB_ERROR_CODES.MaxTimeMSExpired ||
...
function endTransaction(session, commandName, callback) {
    ...
    if (commandName === 'commitTransaction') {
        ...
        if (txnState === TxnState.TRANSACTION_ABORTED) {
            callback(new MongoTransactionError('Cannot call commitTransaction after calling abortTransaction'));
            return;
        }
    }
    else {
        ...
        if (txnState === TxnState.TRANSACTION_COMMITTED ||
            txnState === TxnState.TRANSACTION_COMMITTED_EMPTY) {
            callback(new MongoTransactionError('Cannot call abortTransaction after calling commitTransaction'));
            return;
        }
    }
    ...
    function commandHandler(error, result) {
        ...
        if (error instanceof MongoError) {
            if (error.hasErrorLabel(MongoErrorLabel.RetryableWriteError) ||
    ...
    // send the command
    executeOperation(session.client, new RunAdminCommandOperation(undefined, command, {
        session,
        ...
        if (error instanceof MongoError && error.hasErrorLabel(MongoErrorLabel.RetryableWriteError)) {
            ...
            return executeOperation(session.client, new RunAdminCommandOperation(undefined, command, {
                session,
                readPreference: ReadPreference.primary,
                bypassPinningCheck: true
            }), commandHandler);
 ...
 * @internal
 */
export function applySession(session, command, options) {
    var _b, _c;
    if (session.hasEnded) {
...
export function updateSessionFromResponse(session, document) {
    var _b;
